...
public class JsonWebTokenEncriptService implements EncriptService {
	...
	public JsonWebTokenEncriptService(String secret) {
		if (algorithm == null) {
			...
			try {
				...
				throw new AppException(500, 500, "Can not create the jwt algorithm", new Object(), e);
			}
		}
		if (jwtVerifier == null) {
	...
	public UserIdentification decript(String token) {
		try {
			...
			UserIdentification response = UserIdentification.builder().userId(new UserId(decoded.getSubject()))
					.role(new Role(decoded.getClaim("role").asString())).build();

			logger.info("LoginResponse: " + new Gson().toJson(response));
			return response;

		} catch (TokenExpiredException tee) {
			throw new LoginException(ErrorConstantsShared.TOKEN_EXPIRED, "Bad credentials");
		} catch (JWTVerificationException e) {
			throw new LoginException(401, "Bad credentials");
		}
	}
	...
	public String generate() {
		...
		return SecurityEncription.INSTANCE
				.digest(String.format("%s%s", raw, String.format("%x", System.currentTimeMillis())));
	}
