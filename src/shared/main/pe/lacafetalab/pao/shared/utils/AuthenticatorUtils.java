...
public class AuthenticatorUtils {
	...
	public static String findToken(String header) {
		Optional.ofNullable(StringUtils.defaultIfBlank(header, null))
				.orElseThrow(() -> new LoginException(401, "Invalid credentials, check for token presence"));
		Matcher matcher = AUTH_HEADER_PATTERN.matcher(header);
		if (matcher.find()) {
			return matcher.group(5);
		} else {
			throw new LoginException(401, "Invalid credentials, check for token presence");
		}
	}
