...
import pe.lacafetalab.base.ddd.domain.exception.AppException;
...
public class ExternalHttpServiceImpl implements ExternalHttpService {
	...
	@Override
	public <T, S> ExternalResponse<S> doRequest(String url, String httpMethod, Map<String, String> headers, T body,
			Class<S> target) {
		LOGGER.info("http service request url[{}], payload [{}]", url, body != null ? gson.toJson(body) : null);
		byte[] bytes = null;
		if (body != null) {
			try {
				...
				ex.printStackTrace();
				throw new BadRequestException(400, String.format("Error in external service [%s]", ex.getMessage()));
			}
		}
	...
	@Override
	public <S, T> ExternalResponse<S> doRequest(String url, String httpMethod, Map<String, String> headers,
			byte[] bytes, Class<S> target, Long timeout) {
		try {
			...
			LOGGER.info("http service response done in {} milliseconds  status code [{}] - headers [{}]",
					(System.currentTimeMillis() - start), response.statusCode(), gson.toJson(response.headers()));
			...
			if (responseCode >= 200 && responseCode < 300) {
				...
				AppException error = getResponse(content, AppException.class);
				throw new AppException(responseCode, error.getCode(), error.getMessage(), error.getData(), null);
			} else {
				...
				throw new AppException(responseCode, 400, message, error, null);
			}
	...
	@Override
	public <T> CompletableFuture<HttpResponse<String>> doRequestAsync(String url, String httpMethod, String token,
			T body) {
		...
		try {
			...
			HttpRequestBuilder.uri(URI.create(url)).timeout(Duration.ofMillis(75000)).header("Content-Type",
					"application/json");
			...
			CompletableFuture<HttpResponse<String>> response = client.sendAsync(HttpRequestBuilder.build(),
			...
			throw new BadRequestException(400, String.format("Error in external http service [%s]", ex.getMessage()),
					ex);
		}
	}
